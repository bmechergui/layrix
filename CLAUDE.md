...
User → Sonnet 4.6 (orchestrateur, max 15 itérations, SSE)
  ① call_agent_schema     → Ingénieur Schéma
     Haiku 4.5 → génère Python circuit_synth OU JSON schema
     kicad_gen.py : _generate_with_cs_lib() → circuit_synth pip → .kicad_sch uniquement
  ...
  ④ call_agent_kicad      → Ingénieur Layout (NOUVEAU — séparé du schéma)
...
- Génération dual-mode (schéma seulement — PCB séparé dans call_agent_kicad) :
  - Path A : Haiku génère Python → Docker /schematic/execute → `.kicad_sch`
  - Path B : Haiku génère JSON → `_generate_with_cs_lib()` (circuit_synth pip) → `.kicad_sch`
...
- ✅ **4.x — Pipeline 8 agents experts** (session 2026-05-26) :
  - `call_agent_kicad` créé — sépare génération PCB `.kicad_pcb` de la génération schéma `.kicad_sch`
  ...
  - `orchestrator.ts` : `stepMap` mis à jour (`call_agent_kicad → 'KICAD'`), `pcbStateTools` étendu
